...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        for (i=0;i<3;i++) {
            ...
            /* read receiver raw/rtcm data from input stream  从数据流中读数据*/
        ...
        for (i=0;i<3;i++) {
            if (svr->format[i]==STRFMT_SP3||svr->format[i]==STRFMT_RNXCLK) {
                ...
                /* decode receiver raw/rtcm data 解数据流*/
...
*          int     *strs    I  stream types (STR_???)                数据流类型  8条路
                       ...
                       solopt_t *solopt, stream_t *moni, char *errmsg)
{
    ...
    /* sync input streams 时间同步*/
    strsync(svr->stream,svr->stream+1);
    strsync(svr->stream,svr->stream+2);

    /* write start commands to input streams 将start命令写入输入流*/
    for (i=0;i<3;i++) {
        if (!cmds[i]) continue;
