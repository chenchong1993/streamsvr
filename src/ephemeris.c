...
* galileo, qzss)根据广播星历计算出算信号发射时刻卫星的位置和钟差
* args   : gtime_t time     I   time (gpst)
*          eph_t *eph       I   broadcast ephemeris
*          double *rs       O   satellite position (ecef) {x,y,z} (m)
*          double *dts      O   satellite clock bias (s)
...
/* select ephememeris
从导航数据中选择星历。当 iode>=0时，选择与输入期号相同的星历；否则，选择 toe值与星历选择时间标准 time最近的那个星历。
--------------------------------------------------------*/
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    /*根据该卫星所属的导航系统给与星历参考时间的最大时间间隔 tmax赋予相应的值。*/
    sys=satsys(sat,NULL);
    switch (sys) {
    ...
    for (i=0;i<nav->n;i++) {
        ...
        /*接着确保星历选择时间与代查找星历的参考时间的间隔是否小于 tmax。*/
        if ((t=fabs(timediff(nav->eph[i].toe,time)))>tmax) continue;
       /* 如果此时对于输入的期号，有 iode>=0，则该星历就是所要寻找的星历；*/
        if (iode>=0) return nav->eph+i;
        /*否则，验证 t是否满足 t<=tmin。满足的话，就令 tmin=t，该星历目前是距离参考时间最近的*/
        if (t<=tmin) {j=i; tmin=t;} /* toe closest to time */
    }
    /*遍历完导航数据中的所有星历。如果都没有符合条件的，就输出信息并返回 NULL；否则，返回所查找到的星历。*/
    if (iode>=0||j<0) {
                  ...
                  double *dts)
{
    ...
    /*首先调用 satsys函数，根据卫星编号确定该卫星所属的导航系统和该卫星在该系统中的 PRN编号。*/
    sys=satsys(sat,NULL);
    /*对于 GPS导航系统，调用 seleph函数来选择 toe值与星历选择时间标准 teph最近的那个星历。*/
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    *svh=-1;
    /*如果该卫星属于以下系统，则调用 seleph函数来选择广播星历。*/
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
                  ...
                  int *svh)
{
    ...
    *svh=0;
    /*判断星历选项的值，如果是 EPHOPT_BRDC，调用 ephpos函数，根据广播星历计算出算信号发射时刻卫星的 P、V、C*/
    switch (ephopt) {
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        /*用 信号发射时间减去 钟差，得到 GPS时间下的卫星信号发射时间。*/
        time[i]=timeadd(time[i],-dt);

        /* satellite position and clock at transmission time
        调用 satpos函数，计算信号发射时刻卫星的 P(ecef,m)、V(ecef,m/s)、C((s|s/s))。
        ...
        如果计算出的钟差为 0，就再次调用 ephclk函数，将其计算出的卫星钟偏作为最终的结果。*/
        if (dts[i*2]==0.0) {
